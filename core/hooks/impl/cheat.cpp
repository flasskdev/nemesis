...

    {
...

        {
...

            {
                ...
                const float before_rage_yaw = angle_base->viewangles( )
                    ? angle_base->viewangles( )->y( ) : systems::g_input.get_view_angles( ).y;
                features::combat::g_rage.on_create_move( current_cmd );
                systems::g_input.rebase_movement( current_cmd, before_rage_yaw );
                ...
                const float before_legit_yaw = angle_base->viewangles( )
                    ? angle_base->viewangles( )->y( ) : systems::g_input.get_view_angles( ).y;
                features::combat::g_legit.on_create_move( current_cmd );
                systems::g_input.rebase_movement( current_cmd, before_legit_yaw );
        ...
        // Each angle writer has already rebased movement at its own boundary.
        // Do not rotate the final airstrafe/quickpeek result a second time.
...
bool __fastcall cheat::get_transforms_for_hitbox_list( std::uintptr_t a1, std::uintptr_t a2, int* a3 )
{
    ...
    for ( auto i = 0; i < count; ++i )
    {
        const auto transform_ptr = output_array + 32ull * i;
        const auto bone_index = memory::call<int>( get_bone_index, model, shape_array + 16ull * i );
        if ( bone_index >= 0 && bone_index < 256 )
        {
            const auto bone_data = memory::read<systems::bones::data>( target_bone_cache + sizeof( systems::bones::data ) * bone_index );
            memory::write<systems::bones::data>( transform_ptr, bone_data );
        }
    }
...
{
    features::esp::player::g_chams.on_sort_primitives( a2, a4 );
...
{
    return m_get_inaccuracy.call<float>( thisptr, a2, a3 );
}
...
float* __fastcall cheat::get_interpolated_shoot_position( std::uintptr_t thisptr, float* out, int* tick_frac )
{
    ...
    if ( result )
    {
        features::misc::g_impacts.on_get_interpolated_shoot_position( thisptr, result );
    }
    return result;
}
...
std::uintptr_t __fastcall cheat::level_initialization( std::uintptr_t a1, const char* new_map )
{
    if ( new_map )
    {
        rendering::widgets::s_map_name = new_map;
    }
...
{
    rendering::widgets::s_map_name.clear( );
...
void __fastcall cheat::process_input_event( std::uintptr_t thisptr, int slot, float frametime )
{
    m_process_input_event.call<void>( thisptr, slot, frametime );
    systems::g_legit_input.on_process_input_event( thisptr, slot );
}
...
std::uintptr_t __fastcall cheat::render_smoke_map( std::uintptr_t thisptr, std::size_t size, std::uintptr_t* out_ptr )
{
    ...
    if ( out_ptr )
    {
        features::world::g_smoke.on_map( result, size, *out_ptr );
    }
    return result;
}
...
{
    if ( a5 && settings::g_misc.m_removals.flash_alpha.value < 255.0f )
    {
        const auto alpha_factor = std::clamp( settings::g_misc.m_removals.flash_alpha.value / 255.0f, 0.0f, 1.0f );
        a5->m128_f32[ 3 ] *= alpha_factor;
    }
