...
namespace features::combat {
    ...
    void misc::antiaim::on_create_move(systems::input::usercmd* cmd)
    {
        ...
        // Handle manual direction conflicts more efficiently
        ...
        // Set yaw side based on manual input
        ...
        // Calculate and apply new angles with improved stability
    ...
    {
        // Validate input pitch
        if (std::isnan(view_pitch) || std::isinf(view_pitch))
            view_pitch = 0.0f;
        ...
        case settings::combat::antiaim::pitch_mode::up:
            return -89.0f;
        default:
            // For none mode, ensure pitch is within valid range
    ...
    float misc::antiaim::get_yaw(const math::vector3& view_angles, const systems::local::snapshot& local)
    {
        ...
        if (!local_game_scene_node)
            return view_yaw; // Return original yaw if we can't get scene node
        ...
        // Improved backstab avoidance with better distance checking
        ...
        // Improved target selection with better threat assessment
        ...
        // Ensure yaw is within valid range
    ...
    void misc::duckpeek::on_create_move(systems::input::usercmd* cmd)
    {
        ...
        else if (wants_shot)
        {
            ...
            const auto base = cmd->csgo_user_cmd.mutable_base();
            auto subtick_moves = base ? base->mutable_subtick_moves() : nullptr;
            if (subtick_moves)
            {
                for (int i = 0; i < subtick_moves->size(); ++i)
                {
                    auto entry = subtick_moves->mutable_at(i);
                    if (entry && (entry->button() & cstypes::command_buttons::in_duck))
                    {
