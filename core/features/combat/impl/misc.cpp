...
namespace features::combat {
	...
	void misc::antiaim::correct_movement (systems::input::usercmd* cmd) {
		...
		// Если airstrafe активен или игрок в воздухе с включенным airstrafe/bhop,
		// движение уже контролируется movement-системой.
		if ( features::movement::g_airstrafe.active_this_tick( ) ) {
			this->m_should_correct = false;
			return;
		}

		const bool in_air = !( systems::g_prediction.pre( ).flags & cstypes::entity_flags::on_ground );

		// При включённом спинботе correct_movement должен работать в воздухе,
		// чтобы скомпенсировать вращение углов и двигать игрока в том направлении,
		// куда он смотрит, а не в сторону спинбота.
		if ( in_air && ( settings::g_movement.airstrafe.value || settings::g_movement.bhop.value ) && !settings::g_combat.m_antiaim.spinbot.value ) {
			this->m_should_correct = false;
			return;
		}
