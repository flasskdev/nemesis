...
namespace features::combat {
	...
	void misc::antiaim::on_create_move(systems::input::usercmd* cmd)
	{
		...
		const auto input_angles = systems::g_input.get_view_angles();
		this->m_reference_yaw = input_angles.y;
		math::helpers::normalize_angle(this->m_reference_yaw);
		...
		const auto view_angles = input_angles;
	...
	float misc::antiaim::get_yaw(const math::vector3& view_angles, const systems::local::snapshot& local)
	{
		...
		{
			static float spin_offset = 0.0f;

			spin_offset += settings::g_combat.m_antiaim.spin_speed.value;
			math::helpers::normalize_angle(spin_offset);

			auto yaw = view_angles.y + spin_offset;
	...
	{
		if (!this->m_should_correct && !this->m_antiaim_active)
		{
			return;
		}
		...
		float final_yaw = this->m_modified_angles.y;
		...
		math::helpers::normalize_angle(final_yaw);

		float reference_yaw = this->m_reference_yaw;
		math::helpers::normalize_angle(reference_yaw);

		const auto rotation_deg = math::helpers::normalize_yaw(reference_yaw - final_yaw);

		if (std::fabsf(rotation_deg) < 0.01f)
		{
			return;
		}

		const auto rot = rotation_deg * (std::numbers::pi_v<float> / 180.0f);
		const auto cos_rot = std::cosf(rot);
		const auto sin_rot = std::sinf(rot);

		const auto forward_move = base->forwardmove();
		const auto side_move = base->leftmove();

		const auto corrected_forward = forward_move * cos_rot - side_move * sin_rot;
		const auto corrected_side = forward_move * sin_rot + side_move * cos_rot;
		...
		for (auto i = 0; i < base->subtick_moves_size(); ++i)
		{
			...
			if (step->m_has_bits.test(0x8) || step->m_has_bits.test(0x10))
			{
				...
				step->set_analog_forward_delta(std::clamp(
					fwd_delta * cos_rot - left_delta * sin_rot,
					-2.0f,
					2.0f));

				step->set_analog_left_delta(std::clamp(
					fwd_delta * sin_rot + left_delta * cos_rot,
					-2.0f,
					2.0f));
			}
		}
	...
	{
		// TODO: нормальная проверка лестницы.
		// Пока оставляем false, чтобы anti-aim не отключался из-за пустой заглушки.
		(void)local_pawn;
		return false;
	}
	...
	void misc::autostop::on_create_move(systems::input::usercmd* cmd)
	{
		...
		float reference_yaw = systems::g_input.get_view_angles().y;
		math::helpers::normalize_angle(reference_yaw);

		const auto yaw_rad = reference_yaw * (std::numbers::pi_v<float> / 180.0f);
